...
export const bleGetService = async (server: BluetoothRemoteGATTServer): Promise<BluetoothRemoteGATTService> => {
    const service: BluetoothRemoteGATTService = await server.getPrimaryService(SERVICE_UUID)
    console.log(service)
    return service
}

export const bleGetChar = async (service: BluetoothRemoteGATTService): Promise<BluetoothRemoteGATTCharacteristic> => {
    const char: BluetoothRemoteGATTCharacteristic = await service.getCharacteristic(CHAR_UUID)
    console.log(char)
    return char
}

export const bleGetCharServer = async (server: BluetoothRemoteGATTServer): Promise<BluetoothRemoteGATTCharacteristic | null> => {
    const char: BluetoothRemoteGATTCharacteristic | null = await server.getPrimaryService(SERVICE_UUID).then( async (service: BluetoothRemoteGATTService) => {
